// src/temperaturePage/components/CombinedChart.jsx
...
export default function TrendChart() {
  ...
  const derived = useMemo(() => {
    ...
    if (granularity === "monthly") {
      ...
      const datasets = [
        {
          ...
          data: monthlySeries.map((r) =>
            r.temp_c === null ? null : Number(r.temp_c.toFixed(2))
          ),
      ...
      return {
        ...
        labels,
        datasets,
        title: `Monthly Average Temperature (Comparison ${year}${
    ...
    if (granularity === "daily") {
      if (yearsLoading || dailyLoading || !dailySeries)
        return { loading: true };
      ...
      const datasets = [
        {
          label: `Avg °C — ${year}-${String(month).padStart(2, "0")}`,
      ...
      if (compareMode && dailySeries2) {
        datasets.push({
          label: `Avg °C — ${year}-${String(compareMonth).padStart(
            2,
            "0"
          )}`,
      ...
      return {
        ...
        title: `Daily Temperature (Comparison ${
          ...
          compareMode
            ? ` vs ${year}-${String(compareMonth).padStart(2, "0")}`
            : ""
        })`,
        xTitle: "Day",
      };
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: derived.title || "Temperature",
      },
    },
    scales: {
      x: {
